...
original <- NConvertClass(cars)
dataset <- NPREPROCESSING_dataset(original, TRUE)
dataset<- NPREPROCESSING_splitdataset(dataset)
measures <- fullDT(train = datatset$train, test = dataset$test, boost = BOOST)
